	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			...
			if (mViewSwitchListener != null) {
				int width = getWidth();
				int position = 0;
				if (hPerceived < width / 2) {
					position = 0;
				} else if (hPerceived < width + width / 2) {
					position = 1;
				} else if (hPerceived < width + width + width / 2) {
					position = 2;
				} else if (hPerceived < width + width + width + width / 2) {
					position = 3;
				}
				// Log.e("ViewFlow", "width=" + width + " xPos=" + hPerceived
				// + " position=" + position);
				mViewSwitchListener.onScrollTo(position);
			}
		}
	}
