...
public final class HttpRequest {
	 ...
	 * 
	 * @param log
	 *            ACRALog to use for all HttpRequest instances.
	 */
	...
	public void sendPost(URL url, Map<?, ?> parameters) throws IOException {
		...
		if (ACRA.DEV_LOGGING)
			log.d(ACRA.LOG_TAG, "HttpPost params : ");
		for (final Object key : parameters.keySet()) {
			if (ACRA.DEV_LOGGING)
				log.d(ACRA.LOG_TAG, " key : '" + key + "'    value: '"
						+ parameters.get(key) + "'");
		}
		...
		if (response != null) {
			...
			if (statusLine != null) {
				...
				if (statusCode.startsWith("4") || statusCode.startsWith("5")) {
					if (ACRA.DEV_LOGGING)
						log.d(ACRA.LOG_TAG, "Could not send HttpPost : "
								+ httpPost);
			...
			if (ACRA.DEV_LOGGING)
				log.d(ACRA.LOG_TAG, "HttpResponse Status : "
