...
public final class NetHelper {
	...
	public static DefaultHttpClient getHttpClient() {
		...
		client.setHttpRequestRetryHandler(new RequestRetryHandler(RETRY_COUNT));
		return client;
	}
	...
	// public static void signOAuth(final String host, final String path, final
	// String method, final boolean https, final Parameters params, final String
	// token, final String tokenSecret) {
	// params.put(
	...
	// paramsEncoded.add(nameValue.getName() + "=" +
	// cgBase.urlencode_rfc3986(nameValue.getValue()));
	// }
	//
	// final String keysPacked = Settings.getKeyConsumerSecret() + "&" +
	// StringUtils.defaultString(tokenSecret); // both even if empty some of
	// them!
	// final String requestPacked = method + "&" +
	// cgBase.urlencode_rfc3986((https ? "https" : "http") + "://" + host +
	// path) + "&" +
	// cgBase.urlencode_rfc3986(StringUtils.join(paramsEncoded.toArray(), '&'));
	// params.put("oauth_signature",
	// cgBase.base64Encode(CryptUtils.hashHmac(requestPacked, keysPacked)));
	// }
	 ...
	 * header.
	 */
	private static class GZIPHttpRequestInterceptor implements
	 ...
	 * encoding header.
	 */
	private static class GZIPHttpResponseInterceptor implements
