...
final class SharedPreferencesCollector {
	...
	public static String collect(Context context) {
		...
		final String[] shrdPrefsIds = ACRA.getConfig()
				.additionalSharedPreferences();
		if (shrdPrefsIds != null) {
			for (final String shrdPrefId : shrdPrefsIds) {
				shrdPrefs.put(shrdPrefId, context.getSharedPreferences(
						shrdPrefId, Context.MODE_PRIVATE));
			}
		}
				...
				if (kv != null && kv.size() > 0) {
					for (final String key : kv.keySet()) {
						result.append(key).append("=")
								.append(kv.get(key).toString()).append("\n");
					}
				} else {
					result.append("empty\n");
				}
