...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
	...
	ErrorReporter(Context context, SharedPreferences prefs, boolean enabled) {
		...
		final String initialConfiguration = ReportUtils
				.getCrashConfiguration(mContext);
	...
	public void uncaughtException(Thread t, Throwable e) {
		...
		if (mReportingInteractionMode == ReportingInteractionMode.SILENT
				|| (mReportingInteractionMode == ReportingInteractionMode.TOAST && ACRA
			...
			Log.e(LOG_TAG,
					mContext.getPackageName() + " fatal error : "
							+ e.getMessage(), e);
	...
	private void deletePendingNonApprovedReports() {
		...
		final int nbReportsToKeep = mReportingInteractionMode == ReportingInteractionMode.NOTIFICATION ? 1
				: 0;
	 ...
	 *            This report is to be sent silently.
	 ...
	 */
	private SendWorker handleException(Throwable e,
		...
		if (reportingInteractionMode == ReportingInteractionMode.TOAST
				|| (reportingInteractionMode == ReportingInteractionMode.NOTIFICATION && ACRA
						.getConfig().resToastText() != 0)) {
			new Thread() {
		...
		final CrashReportData crashReportData = crashReportDataFactory
				.createCrashData(e, isSilentReport);
